...
st.sidebar.markdown("---")
st.sidebar.header("≡ƒôñ Upload Resumes")
...
if not uploaded_files:
    st.info("Γ¼à Upload resumes to start screening.")
    st.stop()

st.sidebar.markdown("---")
st.sidebar.header("Γÿ░ Filters")
...
if "results_df" not in st.session_state:
    ...
    for rank, (score, idx) in enumerate(zip(scores, indices), start=1):
        ...
        ranked_rows.append({
            ...
            "Salary Expectation (Γé╣)": row["Salary Expectation (Γé╣)"],
...
st.subheader("≡ƒæñ Ranked Candidates")
st.dataframe(
    filtered_df[
...
st.markdown("### Γä╣∩╕Å Candidate Details")
...
for _, row in filtered_df.iterrows():
    ...
    with st.expander(f"#{row['Rank']} ΓÇó {row['Name']} ΓÇö {status}"):
        ...
        with col1:
            ...
            st.markdown(f"**Salary Expectation:** {row['Salary Expectation (Γé╣)']}")
...
st.download_button(
    "≡ƒôÑ Download Shortlisted Candidates",
